...
import { Link, Calendar, Mail, LogOut, GraduationCap, BookOpen, HardDrive } from "lucide-react";

export default function SettingsPage() {
  const { moodleToken, zonaSession, zonaProfile, zonaLoading, loadZonaProfile, googleAccessToken, user, logout } = useApp();
  ...
  const connections = [
    ...
    { name: "Zona Interactiva", status: zonaSession ? "Conectado" : "No disponible", Icon: GraduationCap, color: zonaSession ? "#16A34A" : "#D97706", connected: !!zonaSession },
    ...
    <div className="fade-in" style={{ maxWidth: 640 }}>
      ...
      {user && (
        <div style={{ background: P.card, borderRadius: 14, border: `1px solid ${P.border}`, padding: "16px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 14 }}>
          {user.picture ? (
            <img src={user.picture} alt="" style={{ width: 48, height: 48, borderRadius: 12 }} />
          ) : (
