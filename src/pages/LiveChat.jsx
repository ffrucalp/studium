...
export default function LiveChat() {
  ...
  const ensureRooms = useCallback(async () => {
    ...
    try {
      ...
      for (const room of (allRooms || [])) {
        const courseMatch = courses.find(c => c.id === room.course_id);
        if (courseMatch) {
  ...
  // ── Get last message for room preview ──
  ...
  useEffect(() => {
    ...
    const loadCounts = async () => {
      const counts = {};
      for (const room of rooms) {
        const { count } = await supabase.from("chat_members").select("*", { count: "exact", head: true }).eq("room_id", room.id);
